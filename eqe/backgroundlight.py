...
# Initialize TLPMX and Cornerstone_Mono objects
...
def start_measurement():
    ...
    while current_wavelength <= end_wavelength:
        ...
        # Measure power 5 times and calculate the average
        power_values = []
        for _ in range(50):
            power = c_double()
    ...
    while current_wavelength <= end_wavelength:
        ...
        y_values.append(average_power - background_y_values[len(x_values) - 1])  # Subtract background light
...
def save_data():
    ...
    if file_path:
        with open(file_path, mode='w', newline='') as file:
            ...
            writer.writerow(["Wavelength (nm)", "Power Measurements (W)"])
...
fig, ax = plt.subplots()
ax.set_xlabel('Confirmed Mono Wavelength (nm)', fontsize=12)
ax.set_ylabel('Power Measurements (W)', fontsize=12)
ax.set_title('Power Measurements vs. Confirmed Mono Wavelength', fontsize=12)
