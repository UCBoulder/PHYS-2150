/**
 * EQE Measurement Application
 *
 * ES6 module for the EQE characterization interface.
...
// ============================================
// Plot Updates (Called from Python)
// ============================================
...
// ============================================
// Mock Measurements (Offline Mode)
// ============================================
...
// AM1.5G photon flux data (wavelength in nm, flux in photons/m²/s/nm)
// Source: ASTM G173-03 Reference Spectra
...
const AM15G_PHOTON_FLUX = [
    // Photon flux in 10^18 photons/m²/s/nm (divide by 1e18 when using)
...
].map(x => x * 1e18);  // Convert to actual photons/m²/s/nm
...
/**
 * Calculate integrated Jsc from EQE using AM1.5G spectrum
 * Jsc = q × ∫ EQE(λ) × Φ(λ) dλ
 * Returns Jsc in mA/cm²
 */
function calculateJsc(eqeWavelengths, eqeValues) {
    ...
    for (let i = 0; i < eqeWavelengths.length - 1; i++) {
        ...
        const eqe1 = eqeValues[i] / 100;  // Convert % to fraction
        const eqe2 = eqeValues[i + 1] / 100;

        // Get photon flux at these wavelengths (interpolate AM1.5G data)
        ...
        const dLambda = wl2 - wl1;  // Keep in nm (matches flux units: photons/m²/s/nm)

        integral += avgEQE * avgPhi * dLambda;
    }

    // Jsc = q × integral, convert A/m² to mA/cm²
    // 1 A/m² = 0.1 mA/cm²
...
function onCurrentFileSelected(event) {
    ...
    reader.onload = (e) => {
        try {
            ...
            // Extract cell/pixel from current file (it's the cell measurement)
...
function saveAnalysisResults() {
    ...
    // Build CSV content with header summary
...
function setupKeyboardShortcuts() {
    document.addEventListener('keydown', (e) => {
        ...
        // Ctrl+Shift+E - Toggle analysis panel
